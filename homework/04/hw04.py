...
def balanced(m):
    ...
    if is_planet(m):
        return True
    return balanced(end(left(m))) and balanced(end(right(m))) and total_weight(end(left(m)))*length(left(m))==\
...
def quadratic(x, a, b, c):
    ...
    if a==0:return interval(c,c)
    inflex=(-1*b)/(2*a)
    sorted_interval=lambda l,u:interval(l,u) if l<=u else interval(u,l)
    if inflex<=lower_bound(x) or inflex>=upper_bound(x):return sorted_interval(f(lower_bound(x)),f(upper_bound(x)))
    if a>0:return interval(f(inflex),max(f(lower_bound(x)),f(upper_bound(x))))
    return interval(min(f(lower_bound(x)),f(upper_bound(x))),f(inflex))
