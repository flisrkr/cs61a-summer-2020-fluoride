    ...
    def add_to(self, place):
        if place.ant is None:
            ...
            if self.can_contain(place.ant):
                ...
                place.ant=self
            elif place.ant.can_contain(self):place.ant.contain_ant(self)
            else:assert False, 'Two ants in {0}'.format(place)
            # END Problem 9
...
class ThrowerAnt(Ant):
    ...
    def nearest_bee(self, beehive):
        ...
        target_place,probe=self.place,0
        while probe<self.min_range and target_place is not beehive:
            target_place=target_place.entrance
            probe+=1
        while probe<=self.max_range and target_place is not beehive:
            target_bee=rANTdom_else_none(target_place.bees)
            if target_bee:return target_bee
            else:
                target_place=target_place.entrance
                probe+=1
        return None
...
class FireAnt(Ant):
    ...
    def reduce_armor(self, amount):
        ...
        # BEGIN Problem 5
        "*** YOUR CODE HERE ***"
        bees_interface=self.place.bees[:]
        reflect_damage=amount
        if amount>=self.armor:reflect_damage+=self.damage
        for bee in bees_interface:bee.reduce_armor(reflect_damage)
...
class HungryAnt(Ant):
    ...
    def action(self, gamestate):
        ...
        if self.digesting:
            ...
            if target_bee:self.eat_bee(target_bee)
        # END Problem 6
...
class NinjaAnt(Ant):
    ...
    def action(self, gamestate):
        ...
        "*** YOUR CODE HERE ***"
        bees_interface=self.place.bees[:]
        for bee in bees_interface:bee.reduce_armor(self.damage)
        # END Problem 7
...
class ContainerAnt(Ant):
    ...
    def action(self, gamestate):
        # BEGIN Problem 9
        "*** YOUR CODE HERE ***"
        if self.contained_ant:self.contained_ant.action(gamestate)
        # END Problem 9
...
# END Problem 13
    ...
    def __init__(self, armor=1):
        # BEGIN Problem 13
        "*** YOUR CODE HERE ***"
        self.queen_seq=QueenAnt.queen_attempt
        QueenAnt.queen_attempt+=1
    ...
    def action(self, gamestate):
        ...
        # BEGIN Problem 13
        "*** YOUR CODE HERE ***"
        if self.queen_seq:self.reduce_armor(self.armor)
        else:
            super().action(gamestate)
            probe=self.place.exit
            while probe:
                if probe.ant and not probe.ant.inspired:
                    probe.ant.damage*=2
                    probe.ant.inspired=True
                if isinstance(probe.ant,ContainerAnt) and probe.ant.contained_ant and not probe.ant.contained_ant.inspired:
                    probe.ant.contained_ant.damage*=2
                    probe.ant.contained_ant.inspired=True
                probe=probe.exit
        # END Problem 13
    ...
    def reduce_armor(self, amount):
        ...
        # BEGIN Problem 13
        "*** YOUR CODE HERE ***"
        if self.queen_seq:super().reduce_armor(amount)
        else:
            self.armor-=amount
            if self.armor<=0:bees_win()
        # END Problem 13

    def remove_from(self, place):
        if self.queen_seq:super().remove_from(place)
